...
return [
    ...
    "router"=> [
        ...
        "http"      => [
            ...
            "middleware"    => [
                ...
                //控制器已经执行，即将发送响应数据
                "before_send"       => [
                    //实现 Smarty 模板解析，不执行 Response 默认的模板解析
                    \app\middleware\View::class
                ]
            ]
        ]
    ],
    ...
    "swoole" => [
        ...
        "http"      => [
            ...
            "session" => [
                //驱动，可选：file=临时文件，或 middleware
                "driver"    => "middleware",
                //使用中间键处理
                "middleware" => [
                    "get"   => \app\middleware\Session::class,
                    "set"   => \app\middleware\Session::class,
                    "drop"  => \app\middleware\Session::class
                ],
                //如果是使用临时文件，保存的位置在哪里？
